...
const AdminRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("AdminRegister successfully", response.data)
      toast.success(response.data.message)
      navigate('/admin/login')
    } catch (error) {
      if (error.response) {
        setErrorMessage(error.response.data.errors || "AdminRegister failed!")
      }
    }
  }
      ...
      <div className='h-screen container mx-auto flex items-center justify-center text-white'>
        ...
        <header className='absolute top-0 left-0 w-full flex justify-between items-center p-5'>
          ...
          <div className='flex items-center space-x-4'>
            <Link to={"/admin/login"} className='bg-transparent border border-gray-500 py-2 px-4 rounded-md'>Login</Link>
            <Link to={"/courses"} className='bg-orange-500 py-2 px-4 rounded-md'>Join Now</Link>
          </div>
        </header>

        {/* Sign Up */}
        <div className='bg-gray-900 p-6 rounded-lg shadow-lg w-100 mt-15 mb-15'>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type='submit' className='bg-orange-500 text-white w-full py-3 px-6 rounded-mb transition mt-2 hover:bg-blue-600'>Register</button>
          </form>
        </div>
      </div>
    </div>
